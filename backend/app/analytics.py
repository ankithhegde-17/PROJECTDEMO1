...
class MongoObject:
    ...
    def __getattr__(self, key):
        val = self._d.get(key)
        if key in ("join_date", "date", "application_date") and isinstance(val, str):
            try:
...
def is_mongo(db):
    return hasattr(db, "employees")

def filtered_people(db, department=None, location=None):
    if is_mongo(db):
        query = {}
        if department: query["department"] = department
        if location: query["location"] = location
        return [MongoObject(doc) for doc in db.employees.find(query)]
    q=db.query(Employee)
    if department: q=q.filter(Employee.department==department)
    if location: q=q.filter(Employee.location==location)
    return q.all()

def attendance_rows(db, ids):
    if is_mongo(db):
        return [MongoObject(doc) for doc in db.attendance.find({"employee_id": {"$in": ids}})] if ids else []
    return db.query(Attendance).filter(Attendance.employee_id.in_(ids)).all() if ids else []

def performance_rows(db, ids):
    if is_mongo(db):
        return [MongoObject(doc) for doc in db.performance.find({"employee_id": {"$in": ids}})] if ids else []
    return db.query(Performance).filter(Performance.employee_id.in_(ids)).all() if ids else []
...
def overview(db, department=None, location=None):
    ...
    if is_mongo(db):
        cands = [MongoObject(doc) for doc in db.candidates.find({"department": department} if department else {})]
        depts = sorted(db.employees.distinct("department"))
        locs = sorted(db.employees.distinct("location"))
    else:
        cands = db.query(Candidate).filter(Candidate.department==department).all() if department else db.query(Candidate).all()
        depts = sorted({p.department for p in db.query(Employee).all()})
        locs = sorted({p.location for p in db.query(Employee).all()})
    hired=sum(c.status=="Hired" for c in cands); active_c=sum(c.status=="Active" for c in cands)
